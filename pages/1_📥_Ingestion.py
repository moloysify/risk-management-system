...
def load_ingestion_log(window_hours):
    conn = get_connection()
    try:
        df = pd.read_sql("SELECT * FROM INGESTION_LOG WHERE started_at >= DATEADD('hour', -%s, CURRENT_TIMESTAMP()) ORDER BY started_at DESC", conn, params=(window_hours,))
...
if df.empty:
    ...
    for _, row in topic_status.iterrows():
        ...
        if row["status"] == "error":
            c2.error(f"⚠️ {row['kafka_topic']} error at {row['started_at']}")

    st.header("Records by Data Type")
    st.bar_chart(df.groupby("data_type")["record_count"].sum())

    st.header("Duration (ms) by Data Type")
    st.dataframe(df.groupby("data_type")["processing_duration_ms"].agg(["min","max","mean"]).reset_index(), use_container_width=True)

    st.header("Slow Events (> 5s)")
    slow = df[df["processing_duration_ms"] > SLOW_THRESHOLD_MS]
    if slow.empty:
        st.success("None detected.")
    else:
        st.warning(f"{len(slow)} slow batch(es)")
